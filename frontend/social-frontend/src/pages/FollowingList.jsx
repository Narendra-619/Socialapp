...
export default function FollowingList() {
  ...
  useEffect(() => {
    ...
    const fetchFollowing = async () => {
      try {
        const res = await API.get(`/users/${userId}/following`, { signal: controller.signal });
  ...
  const isOwnProfileList = currentUser && (String(currentUser._id || currentUser.id) === String(userId));

  const handleFollowToggle = async (targetUserId) => {
    setActionLoading(targetUserId);
    try {
      const res = await API.post(`/users/${targetUserId}/follow`);
      if (isOwnProfileList && res.data.isFollowing === false) {
  ...
  return (
    <div className="max-w-xl mx-auto py-6 sm:py-10 px-4 sm:px-0">
      ...
      <div className="mb-8 fade-in">
        ...
        <h1 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tight">Following</h1>
        {loading ? (
          <div className="w-20 h-4 bg-zinc-200 dark:bg-zinc-800 rounded-md animate-pulse mt-2" />
        ) : (
          <p className="text-zinc-500 dark:text-zinc-400 mt-1 font-medium">{following.length} people</p>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 font-medium italic">Not following anyone yet.</p>
        </div>
      ) : (
                ...
                {!isOwn && (
                  <button
                    ...
                    className={`px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 ${
                      u.isFollowing
                        ? "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10"
                        : u.isPending
